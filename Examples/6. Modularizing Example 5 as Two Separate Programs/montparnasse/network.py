...
def get_care_packages():
    ...
    return care_pallet
# ---------------------------------------------------------------------
# Check if model exists, if so, load it. Otherwise, make new networks,
# one for jet, one for partyicle
# ---------------------------------------------------------------------
if os.path.isfile("part_ff_model") and os.path.isfile("jet_ff_model"):
    part_ffmodel = tf.keras.models.load_model("part_ff_model")
    jet_ffmodel = tf.keras.models.load_model("jet_ff_model")
else:
    # Redesign neural network to take in as input 997 indexed jet objects,
    # Consisting of 3x6 dimensional float values per index
    jet_ffmodel = tf.keras.models.Sequential()
    jet_ffmodel.add(tf.keras.layers.Flatten())
    jet_ffmodel.add(tf.keras.layers.Dense(18, activation=tf.nn.relu))
    jet_ffmodel.add(tf.keras.layers.Dense(18, activation=tf.nn.relu))
    jet_ffmodel.add(tf.keras.layers.Dense(18, activation=tf.nn.relu))
    jet_ffmodel.add(tf.keras.layers.Dense(1, activation=tf.nn.sigmoid))

    part_ffmodel = tf.keras.models.Sequential()
    part_ffmodel.add(tf.keras.layers.Flatten())
    part_ffmodel.add(tf.keras.layers.Dense(64, activation=tf.nn.relu))
    part_ffmodel.add(tf.keras.layers.Dense(25, activation=tf.nn.relu))
    part_ffmodel.add(tf.keras.layers.Dense(1, activation=tf.nn.relu))

jet_ffmodel.compile(loss='binary_crossentropy',
                optimizer='Adam',
                metric=['acurracy'])

part_ffmodel.compile(loss='binary_crossentropy',
                optimizer='Adam',
                metric=['acurracy'])
# -----------------------------------------------------------------------

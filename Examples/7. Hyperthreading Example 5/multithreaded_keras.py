...
def is_massless_or_isolated(jet):
    ...
    if len(jet.constituents_array()) == 1: 
        if np.abs(jet.userinfo[pid]) == 22 or np.abs(jet.user(info[pid]) == 21):
            return True
...
# -----------------------------------------------------------------------
# Main process for Spawning Threads
# -----------------------------------------------------------------------
# Spawn the threads
print("Spawning Producer")
thread.start_new_thread(produce, ())
print("Spawning Consumer")
thread.start_new_thread(consume, ())
print("Busy Wait")
while np.load(open("control", "rb")): pass
